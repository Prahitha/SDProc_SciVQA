...
for data in tqdm(ds, total=len(ds)):
    try:
        ...
        print(f"Processed instance {idx} in {elapsed_time:.2f} seconds")
        ...
        print(f"Error processing sample {data.get('instance_id', 'unknown')}: {{{e}}}")
        continue
...
print(f"Finished processing {len(all_data)} samples. Results saved to {output_path}")