    ...
    def __getitem__(self, idx):
        try:
            ...
            if not image_path.exists():
                print(
                    "Warning: Image file {image_path} does not exist. Searching for alternatives...")
...
def encode_image_to_base64(image_path):
    ...
    try:
        ...
        print("Error encoding image {image_path}: {e}")
        return ""
...
def vllm_inference(prompt, image_path, vllm_url="http://localhost:8000/v1/chat/completions"):
    ...
    try:
        ...
        if not image_base64:
            print(
                "Warning: No valid image data for {image_path}. Proceeding with text-only query.")
            # Text-only query
            payload = {
